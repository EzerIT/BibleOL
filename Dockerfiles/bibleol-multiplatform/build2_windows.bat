...
if "%ARCH%"=="arm" (
    set "EMDROS_DIR=emdros_arm"
) else (
    set "EMDROS_DIR=emdros_amd"
)

echo Building image for %ARCH% (%EMDROS_DIR%)...
docker image build --build-arg EMDROS_DIR=%EMDROS_DIR% --build-arg PLATFORM=windows -t tmccormack14/bibleol-windows:2026_04_27 .
