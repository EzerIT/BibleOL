...
\HOrG{#1}{Sometimes clauses (or other parts of a sentence) can contain other clauses inside them. An example
  ...
  clause \heb{וַיַּבְדֵּ֗ל בֵּ֤ין הַמַּ֨יִם֙ וּבֵ֣ין הַמַּ֔יִם} is split in two and contains the clause \heb{אֲשֶׁר֙ מִתַּ֣חַת לָרָקִ֔יעַ} inside it.
...
The first noun, \HOrG{#1}{\heb{רָקִ֖יעַ}}{ἀνέμοις}, is shown in the top line of the answer box, and you
...
\HOrG{#1}{\heb{תֹ֣וךְ}}{θαλάσσῃ}, by clicking the \raisebox{-0.1\baselineskip}{\includegraphics[height=9pt]{right-angle.png}} symbol at the right side of the answer box.
...
When the last word in the sentence, \HOrG{#1}{\heb{מָּ֑יִם}}{γαλήνη}, is shown (Figure \ref{fig-\hebgr-exer-ii}), three
...
first clause is \HOrG{#1}{\heb{הִנְּךָ֥ מֵת֙ עַל־הָאִשָּׁ֣ה}}{Τὸν ἄρτον ἡμῶν τὸν ἐπιούσιον}, and you must decide
...
next clause in the sentence, which is \HOrG{#1}{}{simply the word} \HOrG{#1}{\heb{וְהִ֖וא בְּעֻ֥לַת
    בָּֽעַל}}{ἡμῖν}. \HOrG{#1}{(The middle clause in the sentence (\heb{אֲשֶׁר־לָקַ֔חְתָּ}) has a type that is not
...
\subsection{Disabling ``Locate''}\label{sec-disabling-locate}\index{disabling locate}\index{locate}

A teacher may disable the ``Locate'' button in an exercise.


\subsection{Sentence Context}\label{sec-sentence-context}\index{sentence context}\index{context}
...
\subsection{Fixed Exercises}\label{sec-fixed-exercises}\index{order of exercises}\index{exercises!fixed}\index{exercises!order}
...
\renewcommand{\hebgrlong}{\HOrG{#1}{Hebrew}{Greek}}
%
\section{Creating \hebgrlong{} Exercises}\label{sec-create-\hebgr-exercises}

\subsection{A Simple \hebgrlong{} Exercise}\label{sec-create-simple-\hebgr-exercise}

We shall create an exercise in the conjugation of \hebgrlong{} verbs in the \HOrG{#1}{qatal form of
  the qal stem}{present tense}. We shall ask the user to identify the \HOrG{#1}{person, gender, and
...
\subsubsection{The ``Description'' Tab}\index{description tab@``Description'' tab}\index{tabs!``Description''}

The ``Description'' tab is displayed when you start editing an exercise. Below the row of tabs, you
see a text editing field in which you can write information and instructions to the students who
will be running this exercise. You may, for example, write something like what you see in Figure
\ref{fig-\hebgr-description-tab}.
\begin{figure}
  \begin{center}
    \includegraphics[width=0.7\textwidth]{fig-\hebgr-description-tab.png}
  \end{center}
  \caption{Filling out the description field.}\label{fig-\hebgr-description-tab}
\end{figure}

\subsubsection{The ``Passages'' Tab}\index{passages tab@``Passages'' tab}\index{tabs!``Passages''}

Select the ``Passages'' tab and indicate which part of the \HOrG{#1}{Old}{New} Testament you want to
...
\begin{itemize}
\item Should the ``Locate'' button be shown or not? (See Section \ref{sec-disabling-locate}.)
...
\subsubsection{The ``Sentences'' Tab}\index{sentences tab@``Sentences'' tab}\index{tabs!``Sentences''}
...
\subsubsection{The ``Features'' Tab}\index{features tab@``Features'' tab}\index{tabs!``Features''}
