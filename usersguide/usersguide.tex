...
\begin{comment}
%%% NOTE: Do not enable this section before the working of the timeout dialog has been corrected.
  
\section{Exercise Timeout}\index{timeout}\index{exercises!timeout}

Since some teachers keep track of the time spent on exercises, it is important that you cannot
simply start an exercise and then leave the computer for an extended time. For that reason, if you
spend more than 10 minutes on a question, a warning dialog will pop up to check that you are
still there. (See Figure \ref{fig-timeout}.)
\begin{figure}
  \begin{center}
    \includegraphics[width=0.5\textwidth]{fig-timeout.png}
  \end{center}
  \caption{Timeout dialog.}\label{fig-timeout}
\end{figure}
If you press the ``Go on!'' button, you are given another 10 minutes of exercise time, but if you do
not press the  ``Go on!'' button within 28 seconds of its appearance, the ``Next'' button will be
removed, making it impossible for you to proceed to other questions.
\end{comment}
