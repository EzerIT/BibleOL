    ...
    try {
      ...
      foreach ($owned_classes as $class_id){
        ...
        foreach ($active_exam_query as $active_exam) {
          ...
          $active_exam->attempts_completed = $latest_attempt->attempt_count;
          if (!$latest_attempt->is_done) {
            $active_exam->in_progress = true;
            $active_exam->attempts_completed--;
          }
      ...
      $this->load->view(
        'view_top1', 
        array(
          'title' => $this->lang->line('active_exams'),
  ...
  public function take_exam(){
    try {
      ...
      if (
        ...
        // array for storing list of exercises that have already been completed 
        // if the user is continuing an exam attempt instead of starting one
