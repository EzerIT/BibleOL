<?php

$comment['1st_choice'] = "";
$format['1st_choice'] = "";
$use_textarea['1st_choice'] = false;

$comment['2nd_choice'] = "";
$format['2nd_choice'] = "";
$use_textarea['2nd_choice'] = false;

$comment['3rd_choice'] = "";
$format['3rd_choice'] = "";
$use_textarea['3rd_choice'] = false;

$comment['4th_choice'] = "";
$format['4th_choice'] = "";
$use_textarea['4th_choice'] = false;

$comment['add_entry_button'] = "";
$format['add_entry_button'] = "";
$use_textarea['add_entry_button'] = false;

$comment['badname'] = "";
$format['badname'] = "";
$use_textarea['badname'] = false;

$comment['cancel_button'] = "";
$format['cancel_button'] = "";
$use_textarea['cancel_button'] = false;

$comment['check_answer'] = "";
$format['check_answer'] = "";
$use_textarea['check_answer'] = false;

$comment['clear_all'] = "";
$format['clear_all'] = "";
$use_textarea['clear_all'] = false;

$comment['clear_button'] = "";
$format['clear_button'] = "";
$use_textarea['clear_button'] = false;

$comment['clear_grammar'] = "";
$format['clear_grammar'] = "";
$use_textarea['clear_grammar'] = false;

$comment['click_for_document'] = "";
$format['click_for_document'] = "";
$use_textarea['click_for_document'] = false;

$comment['click_for_picture'] = "";
$format['click_for_picture'] = "";
$use_textarea['click_for_picture'] = false;

$comment['click_for_pictures'] = "";
$format['click_for_pictures'] = "";
$use_textarea['click_for_pictures'] = false;

$comment['click_for_video'] = "";
$format['click_for_video'] = "";
$use_textarea['click_for_video'] = false;

$comment['click_for_web_site'] = "";
$format['click_for_web_site'] = "";
$use_textarea['click_for_web_site'] = false;

$comment['dont_care'] = "";
$format['dont_care'] = "";
$use_textarea['dont_care'] = false;

$comment['dont_show'] = "";
$format['dont_show'] = "";
$use_textarea['dont_show'] = false;

$comment['error_response'] = "";
$format['error_response'] = "";
$use_textarea['error_response'] = false;

$comment['feature'] = "";
$format['feature'] = "";
$use_textarea['feature'] = false;

$comment['feature_prompt'] = "";
$format['feature_prompt'] = "";
$use_textarea['feature_prompt'] = false;

$comment['feature_specification'] = "Dialog box title";
$format['feature_specification'] = "";
$use_textarea['feature_specification'] = false;

$comment['friendly_featsel_prompt'] = "";
$format['friendly_featsel_prompt'] = "";
$use_textarea['friendly_featsel_prompt'] = false;

$comment['glosses'] = "";
$format['glosses'] = "";
$use_textarea['glosses'] = false;

$comment['high_value_prompt'] = "";
$format['high_value_prompt'] = "";
$use_textarea['high_value_prompt'] = false;

$comment['import_shebanq'] = "";
$format['import_shebanq'] = "";
$use_textarea['import_shebanq'] = false;

$comment['item_number'] = "";
$format['item_number'] = "";
$use_textarea['item_number'] = false;

$comment['limited'] = "";
$format['limited'] = "";
$use_textarea['limited'] = false;

$comment['low_value_prompt'] = "";
$format['low_value_prompt'] = "";
$use_textarea['low_value_prompt'] = false;

$comment['missing_filename'] = "";
$format['missing_filename'] = "";
$use_textarea['missing_filename'] = false;

$comment['mql_featsel_prompt'] = "";
$format['mql_featsel_prompt'] = "";
$use_textarea['mql_featsel_prompt'] = false;

$comment['mql_qosel_prompt'] = "";
$format['mql_qosel_prompt'] = "";
$use_textarea['mql_qosel_prompt'] = false;

$comment['multiple_choice'] = "";
$format['multiple_choice'] = "";
$use_textarea['multiple_choice'] = false;

$comment['no'] = "";
$format['no'] = "";
$use_textarea['no'] = false;

$comment['none_of_these'] = "";
$format['none_of_these'] = "";
$use_textarea['none_of_these'] = false;

$comment['not_integer'] = "";
$format['not_integer'] = "";
$use_textarea['not_integer'] = false;

$comment['no_focus'] = "";
$format['no_focus'] = "";
$use_textarea['no_focus'] = true;

$comment['no_passages'] = "";
$format['no_passages'] = "";
$use_textarea['no_passages'] = false;

$comment['no_request_feature'] = "";
$format['no_request_feature'] = "";
$use_textarea['no_request_feature'] = false;

$comment['no_show_feature'] = "";
$format['no_show_feature'] = "";
$use_textarea['no_show_feature'] = false;

$comment['OK_button'] = "";
$format['OK_button'] = "";
$use_textarea['OK_button'] = false;

$comment['omit_qere'] = "";
$format['omit_qere'] = "";
$use_textarea['omit_qere'] = false;

$comment['or_write_preferred'] = "";
$format['or_write_preferred'] = "";
$use_textarea['or_write_preferred'] = false;

$comment['other_sentence_unit_types'] = "";
$format['other_sentence_unit_types'] = "";
$use_textarea['other_sentence_unit_types'] = false;

$comment['other_value'] = "";
$format['other_value'] = "";
$use_textarea['other_value'] = false;

$comment['passage_selection'] = "Dialog box title";
$format['passage_selection'] = "";
$use_textarea['passage_selection'] = false;

$comment['request'] = "";
$format['request'] = "";
$use_textarea['request'] = false;

$comment['save'] = "";
$format['save'] = "";
$use_textarea['save'] = false;

$comment['select_1_or_more'] = "";
$format['select_1_or_more'] = "";
$use_textarea['select_1_or_more'] = false;

$comment['sending_statistics'] = "";
$format['sending_statistics'] = "";
$use_textarea['sending_statistics'] = false;

$comment['sentence_selection_imported'] = "";
$format['sentence_selection_imported'] = "";
$use_textarea['sentence_selection_imported'] = false;

$comment['sentence_unit_type_prompt'] = "";
$format['sentence_unit_type_prompt'] = "";
$use_textarea['sentence_unit_type_prompt'] = false;

$comment['separate_lines'] = "";
$format['separate_lines'] = "";
$use_textarea['separate_lines'] = false;

$comment['set_all'] = "";
$format['set_all'] = "";
$use_textarea['set_all'] = false;

$comment['set_aramaic'] = "";
$format['set_aramaic'] = "";
$use_textarea['set_aramaic'] = false;

$comment['set_hebrew'] = "";
$format['set_hebrew'] = "";
$use_textarea['set_hebrew'] = false;

$comment['show'] = "";
$format['show'] = "";
$use_textarea['show'] = false;

$comment['show_answer'] = "";
$format['show_answer'] = "";
$use_textarea['show_answer'] = false;

$comment['show_border'] = "";
$format['show_border'] = "";
$use_textarea['show_border'] = false;

$comment['show_qere'] = "";
$format['show_qere'] = "";
$use_textarea['show_qere'] = false;

$comment['unlimited'] = "";
$format['unlimited'] = "";
$use_textarea['unlimited'] = false;

$comment['use_for_qosel'] = "";
$format['use_for_qosel'] = "";
$use_textarea['use_for_qosel'] = false;

$comment['use_qo_selection'] = "{0} will be replaced by MQL code";
$format['use_qo_selection'] = "";
$use_textarea['use_qo_selection'] = false;

$comment['verb_class'] = "";
$format['verb_class'] = "";
$use_textarea['verb_class'] = false;

$comment['verb_class_dont_care'] = "";
$format['verb_class_dont_care'] = "";
$use_textarea['verb_class_dont_care'] = false;

$comment['verb_class_no'] = "";
$format['verb_class_no'] = "";
$use_textarea['verb_class_no'] = false;

$comment['verb_class_yes'] = "";
$format['verb_class_yes'] = "";
$use_textarea['verb_class_yes'] = false;

$comment['visual'] = "";
$format['visual'] = "";
$use_textarea['visual'] = false;

$comment['word_frequency_color_limit'] = "";
$format['word_frequency_color_limit'] = "";
$use_textarea['word_frequency_color_limit'] = false;

$comment['word_spacing'] = "";
$format['word_spacing'] = "";
$use_textarea['word_spacing'] = false;

$comment['yes'] = "";
$format['yes'] = "";
$use_textarea['yes'] = false;

