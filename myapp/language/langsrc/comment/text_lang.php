<?php

$comment['book_prompt'] = "";
$format['book_prompt'] = "";
$use_textarea['book_prompt'] = false;

$comment['cannot_write_to_quiz_file'] = "";
$format['cannot_write_to_quiz_file'] = "";
$use_textarea['cannot_write_to_quiz_file'] = false;

$comment['chapter'] = "";
$format['chapter'] = "";
$use_textarea['chapter'] = false;

$comment['chapter_prompt'] = "";
$format['chapter_prompt'] = "";
$use_textarea['chapter_prompt'] = false;

$comment['click_folder'] = "";
$format['click_folder'] = "";
$use_textarea['click_folder'] = true;

$comment['context_sentences'] = "";
$format['context_sentences'] = "";
$use_textarea['context_sentences'] = false;

$comment['corpus_copyright'] = "HTML";
$format['corpus_copyright'] = "keep_blanks";
$use_textarea['corpus_copyright'] = true;

$comment['corpus_copyright_title'] = "";
$format['corpus_copyright_title'] = "";
$use_textarea['corpus_copyright_title'] = false;

$comment['corpus_prompt'] = "";
$format['corpus_prompt'] = "";
$use_textarea['corpus_prompt'] = false;

$comment['description'] = "";
$format['description'] = "";
$use_textarea['description'] = false;

$comment['directory'] = "Text in title bar";
$format['directory'] = "";
$use_textarea['directory'] = false;

$comment['display'] = "Button text";
$format['display'] = "";
$use_textarea['display'] = false;

$comment['edit_quiz'] = "Text in title bar";
$format['edit_quiz'] = "";
$use_textarea['edit_quiz'] = false;

$comment['enter_filename_no_3et'] = "";
$format['enter_filename_no_3et'] = "";
$use_textarea['enter_filename_no_3et'] = false;

$comment['everything'] = "";
$format['everything'] = "";
$use_textarea['everything'] = false;

$comment['features'] = "";
$format['features'] = "";
$use_textarea['features'] = false;

$comment['file_exists_overwrite'] = "";
$format['file_exists_overwrite'] = "";
$use_textarea['file_exists_overwrite'] = false;

$comment['finish'] = "";
$format['finish'] = "";
$use_textarea['finish'] = false;

$comment['finish_section'] = "";
$format['finish_section'] = "";
$use_textarea['finish_section'] = false;

$comment['finish_no_grading'] = "";
$format['finish_no_grading'] = "";
$use_textarea['finish_no_grading'] = false;

$comment['first_verse'] = "";
$format['first_verse'] = "";
$use_textarea['first_verse'] = false;

$comment['first_verse_prompt'] = "";
$format['first_verse_prompt'] = "";
$use_textarea['first_verse_prompt'] = false;

$comment['fixed_questions'] = "";
$format['fixed_questions'] = "";
$use_textarea['fixed_questions'] = false;

$comment['import_button'] = "";
$format['import_button'] = "";
$use_textarea['import_button'] = false;

$comment['import_from_shebanq'] = "Title in dialog box";
$format['import_from_shebanq'] = "";
$use_textarea['import_from_shebanq'] = false;

$comment['last_verse'] = "";
$format['last_verse'] = "";
$use_textarea['last_verse'] = false;

$comment['last_verse_prompt'] = "";
$format['last_verse_prompt'] = "";
$use_textarea['last_verse_prompt'] = false;

$comment['locate'] = "";
$format['locate'] = "";
$use_textarea['locate'] = false;

$comment['make_a_selection'] = "";
$format['make_a_selection'] = "";
$use_textarea['make_a_selection'] = false;

$comment['may_locate'] = "";
$format['may_locate'] = "";
$use_textarea['may_locate'] = false;

$comment['missing_database_name'] = "";
$format['missing_database_name'] = "";
$use_textarea['missing_database_name'] = false;

$comment['mql_compiler_error_colon'] = "";
$format['mql_compiler_error_colon'] = "";
$use_textarea['mql_compiler_error_colon'] = false;

$comment['mql_database_error_colon'] = "";
$format['mql_database_error_colon'] = "";
$use_textarea['mql_database_error_colon'] = false;

$comment['mql_sentence_unit'] = "Title in dialog box";
$format['mql_sentence_unit'] = "";
$use_textarea['mql_sentence_unit'] = false;

$comment['my_view'] = "";
$format['my_view'] = "";
$use_textarea['my_view'] = false;

$comment['next'] = "";
$format['next'] = "";
$use_textarea['next'] = false;

$comment['not_logged_in'] = "";
$format['not_logged_in'] = "";
$use_textarea['not_logged_in'] = false;

$comment['no_direct_url'] = "";
$format['no_direct_url'] = "";
$use_textarea['no_direct_url'] = false;

$comment['no_sentences_found'] = "";
$format['no_sentences_found'] = "";
$use_textarea['no_sentences_found'] = false;

$comment['no_text_found'] = "";
$format['no_text_found'] = "";
$use_textarea['no_text_found'] = false;

$comment['only_cli'] = "";
$format['only_cli'] = "";
$use_textarea['only_cli'] = false;

$comment['overwrite'] = "Title in dialog box";
$format['overwrite'] = "";
$use_textarea['overwrite'] = false;

$comment['parent'] = "";
$format['parent'] = "";
$use_textarea['parent'] = false;

$comment['passages'] = "";
$format['passages'] = "";
$use_textarea['passages'] = false;

$comment['progress'] = "";
$format['progress'] = "";
$use_textarea['progress'] = false;

$comment['question_order'] = "";
$format['question_order'] = "";
$use_textarea['question_order'] = false;

$comment['question_order_fixed'] = "";
$format['question_order_fixed'] = "";
$use_textarea['question_order_fixed'] = false;

$comment['question_order_random'] = "";
$format['question_order_random'] = "";
$use_textarea['question_order_random'] = false;

$comment['quiz'] = "Text in title bar";
$format['quiz'] = "";
$use_textarea['quiz'] = false;

$comment['quiz_instruct1'] = "";
$format['quiz_instruct1'] = "";
$use_textarea['quiz_instruct1'] = false;

$comment['quiz_instruct2'] = "";
$format['quiz_instruct2'] = "";
$use_textarea['quiz_instruct2'] = false;

$comment['quiz_name'] = "";
$format['quiz_name'] = "";
$use_textarea['quiz_name'] = false;

$comment['restricted_access'] = "";
$format['restricted_access'] = "";
$use_textarea['restricted_access'] = false;

$comment['save_button'] = "";
$format['save_button'] = "";
$use_textarea['save_button'] = false;

$comment['select_a_passage'] = "HTML";
$format['select_a_passage'] = "keep_blanks";
$use_textarea['select_a_passage'] = true;

$comment['select_a_passage_title'] = "";
$format['select_a_passage_title'] = "";
$use_textarea['select_a_passage_title'] = false;

$comment['select_exercise'] = "";
$format['select_exercise'] = "";
$use_textarea['select_exercise'] = false;

$comment['select_folder'] = "";
$format['select_folder'] = "";
$use_textarea['select_folder'] = false;

$comment['select_number_own'] = "";
$format['select_number_own'] = "";
$use_textarea['select_number_own'] = false;

$comment['select_number_preset'] = "";
$format['select_number_preset'] = "";
$use_textarea['select_number_preset'] = false;

$comment['select_passages'] = "";
$format['select_passages'] = "";
$use_textarea['select_passages'] = false;

$comment['select_quiz'] = "";
$format['select_quiz'] = "";
$use_textarea['select_quiz'] = false;

$comment['select_text'] = "Text in title bar";
$format['select_text'] = "";
$use_textarea['select_text'] = false;

$comment['sentences'] = "";
$format['sentences'] = "";
$use_textarea['sentences'] = false;

$comment['sentence_units'] = "";
$format['sentence_units'] = "";
$use_textarea['sentence_units'] = false;

$comment['sent_after'] = "";
$format['sent_after'] = "";
$use_textarea['sent_after'] = false;

$comment['sent_before'] = "";
$format['sent_before'] = "";
$use_textarea['sent_before'] = false;

$comment['shebanq_query_id_prompt'] = "";
$format['shebanq_query_id_prompt'] = "";
$use_textarea['shebanq_query_id_prompt'] = false;

$comment['shebanq_version_prompt'] = "";
$format['shebanq_version_prompt'] = "";
$use_textarea['shebanq_version_prompt'] = false;

$comment['show_link_icons_prompt'] = "";
$format['show_link_icons_prompt'] = "";
$use_textarea['show_link_icons_prompt'] = false;

$comment['show_only_options'] = "";
$format['show_only_options'] = "";
$use_textarea['show_only_options'] = false;

$comment['show_quiz_passages'] = "";
$format['show_quiz_passages'] = "";
$use_textarea['show_quiz_passages'] = false;

$comment['show_text'] = "Text in title bar";
$format['show_text'] = "";
$use_textarea['show_text'] = false;

$comment['specify_file_name'] = "Title in dialog box";
$format['specify_file_name'] = "";
$use_textarea['specify_file_name'] = false;

$comment['start_quiz'] = "";
$format['start_quiz'] = "";
$use_textarea['start_quiz'] = false;

$comment['start_text_reading'] = "";
$format['start_text_reading'] = "";
$use_textarea['start_text_reading'] = false;

$comment['toggle_mql'] = "";
$format['toggle_mql'] = "";
$use_textarea['toggle_mql'] = false;

$comment['try_an_exercise'] = "";
$format['try_an_exercise'] = "";
$use_textarea['try_an_exercise'] = false;

$comment['using_database'] = "";
$format['using_database'] = "";
$use_textarea['using_database'] = false;

$comment['using_database_version'] = "";
$format['using_database_version'] = "";
$use_textarea['using_database_version'] = false;

$comment['valid_chapters'] = "";
$format['valid_chapters'] = "";
$use_textarea['valid_chapters'] = false;

$comment['warning'] = "";
$format['warning'] = "";
$use_textarea['warning'] = false;

$comment['shortcut_heading'] = "";
$format['shortcut_heading'] = "";
$use_textarea['shortcut_heading'] = false;

$comment['shortcut_text'] = "HTML";
$format['shortcut_text'] = "";
$use_textarea['shortcut_text'] = true;
