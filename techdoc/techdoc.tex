...
\begin{itemize}
\item Apache\index{Apache} (web server\index{web server}). I use version 2.4.18.
\item MySQL\index{MySQL} (database system). I use version 5.7.33.
...
Emdros version 3.8.0 can be downloaded from \url{https://emdros.org/download.html}.

Follow the instructions in the \texttt{INSTALL} file that comes with Emdros to compile and install it on your
computer. You will need a C++\index{C++} compiler and several other software tools -- check the
Emdros documentation.

When you compile Emdros, be sure to include support for PHP~7 or PHP~8\index{PHP}, depending on what
version of PHP you are running.

One way to compile and install Emdros on an Ubuntu Linux system is to create a \texttt{.deb}
installation package. This process will automatically include the proper PHP support. See the Emdros
\texttt{INSTALL} file for details.

When you have compiled and installed Emdros, you need to enable access to it in PHP. The correct way
to do this depends on your Apache\index{Apache} and PHP installation. The Emdros installation
process should have placed a file called
\texttt{EmdrosPHP7.ini} or \texttt{EmdrosPHP8.ini} in the directory
\texttt{/etc/php/*/mods-available}. In order to enable Emdros, you will need to run this command
(replacing 7.1 and PHP7 with the appropriate PHP version):
...
runtime system. The files to copy will be typically found in \texttt{/usr/local/bin} and
...
\item PHP\index{PHP}. I use version 7.4.\footnote{The current main installation on
