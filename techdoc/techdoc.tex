...
\parbox[t]{3cm}{quiz\index{quiz|hyperit}\par exercise\index{exercise|hyperit}} & These terms are
synonymous. They refer to an exercise executed by a user.\\

\midrule

quiz template\index{quiz template|hyperit}\index{template|hyperit} & A file located in the
...
\midrule
\parbox[t]{3cm}{sentence unit\index{sentence unit|hyperit}\par quiz object\index{quiz
    object|hyperit}\par question object\index{question object|hyperit}} & These terms are
...
\parbox[t]{3cm}{passages\index{passages|hyperit}\par universe\index{universe|hyperit}} & The
...
grammar selection box\index{grammar selection box|hyperit}\index{selection box, grammar|hyperit} & A
...
grammar information box\index{grammar information box|hyperit}\index{information box, grammar|hyperit} &
...
grammar hierarchy\index{grammar hierarchy|hyperit}\index{hierarchy, grammar|hyperit} & The
...
\caption{The grammar selection box\index{grammar selection box}\index{selection box, grammar} and
  the grammar information box\index{grammar information box}\index{information box,
    grammar}.}\label{fig-grammar-info}
\end{figure}
...
need a C++\index{C++} compiler and the Gnu make\index{make}\index{Gnu make}\index{gmake} system (and
...
Create an empty database\index{database, user}\index{user database} in MySQL. Then copy the file
...
\$config['mql\_driver']\index{MQL driver}\index{driver, MQL} & Set to \texttt{'native'} to select a
...
\$config['mail\_sender\_address']\index{email address}\index{address, email} & The email address to
be used as sender when Bible OL sends email to users.\\

\$config['mail\_sender\_name']\index{email sender}\index{sender, email} & The name to be used as
sender when Bible OL sends email to users.\\

\$config['google\_login\_enabled']\index{Google!login}\index{login!Google} & Set to \emph{true} if
...
\$config['google\_client\_id']\index{Google!client ID}\index{client ID, Google} & Your Google apps
client ID. Used only if you enable Google login.\\

\$config['google\_client\_secret']\index{Google!client secret}\index{client secret, Google} & Your
...
\subsection{Installing Tsc\index{tsc!installation}\index{TypeScript!compiler installation}}\label{sec-installing-tsc}\label{sec-install-end}
 
\emph{Tsc}\index{tsc}\index{TypeScript!compiler} is the TypeScript\index{TypeScript} compiler. It
...
\chapter{High-level System Architecture}\index{architecture|hyperit}\index{system architecture|hyperit}
...
\begin{itemize}
\item The user database\index{user database}\index{database, user}, which contains information about
  registered users.
...
The client is a web browser\index{web browser}\index{browser} that accesses the server over an
...
Associated with each object are a number of \emph{features}\index{feature
  (Emdros)|hyperit}\index{Emdros!feature|hyperit} that describe various characteristics of the object. For a
...
ID\_D\index{ID\_D|hyperit}\index{Emdros!ID\_D|hyperit}. Like a monad, the ID\_D is an integer, but
...
quizzes & Quiz templates\index{quiz template}\index{template} available for the user. This directory
...
quiz\_templates & Sample quiz templates\index{quiz template}\index{template} to be copied to the
...
virtualkeyboard\index{virtualkeyboard} & A JavaScript-based virtual keyboard\index{virtual
  keyboard}\index{keyboard, virtual} for typing Greek and Hebrew in the client.
...
login\index{Google!login}\index{login!Google} button.
...
%%%%%%%%%%%%%%%%%%%% The visual Feature %%%%%%%%%%%%%%%%%%%%
\section{The \emph{visual}\index{visual feature@``visual'' feature} Feature}\label{sec-visual}\index{visual|hyperit}
...
\item Hebrew/Aramaic characters encoded in UTF-8\index{UTF-8}\index{Unicode}. (In the following
  text, this will be known as the \emph{native} alphabet.)\index{native
    alphabet}\index{alphabet!native}
\item Latin transliteration of the text, encoded in UTF-8\index{UTF-8}\index{Unicode}. (In the
  following text, this will be known as the \emph{transliterated} alphabet.)\index{transliterated
    alphabet}\index{alphabet!transliterated}
\item Hebrew/Aramaic characters int \emph{ETCBC4 transcription}.\index{transcribed
  alphabet}\index{alphabet!transcribed} This transcription is defined in the document
...
\section{A Note on Greek Accents in Unicode}\index{Unicode}\index{Greek accents}\index{accents!Greek}

Classical Greek used polytonic accents\index{polytonic accents}\index{accents!polytonic} over vowels. For example, the letter
alpha could have these polytonic accents:

\begin{my-tabu}{cll}{ \headiii{Character}{Greek accent name}{English accent name} }
    ά & Oxia\index{oxia} (οξεία) & Acute\index{acute accent}\\
    ὰ & Varia\index{varia} (βαρεία) & Grave\index{grave accent}\\
    ᾶ & Perispomeni\index{perispomeni} (περισπωμένη) & Circumflex\index{circumflex accent}\\
\end{my-tabu}

Modern Greek is not a polytonic language, and in 1982 the polytonic accents were replaced by a
single, monotonic accent\index{accents!monotonic}: the \emph{tonos}\index{tonos} (τόνος). In the early years of
...
\chapter{Database Description Files}\index{Database Description File}\index{Description File, Database}<HERTIL>
...
are collectively known as the \emph{Database Description Files}\index{Database Description File}.
...
is typically the character \heb{׃} \emph{(sof pasuq)} or \heb{־} \emph{(maqaf),}\index{maqaf} but it may also include
