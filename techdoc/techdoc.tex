...
\newfontfamily{\dvsm}{DejaVu Sans Mono}
...
PLOT. The project became known as EuroPLOT\index{EuroPLOT}, and 3ET changed its name to
\emph{PLOTLearner.}
...
\begin{itemize}
\item Apache\index{Apache} (web server\index{web server}). I use version 2.4.52.
\item MySQL\index{MySQL} (database system). I use version 8.0.34.
\item SQLite3\index{SQLite3} (database system used by Emdros). I use version 3.37.2.
\item PHP\index{PHP}. I use version 8.1.
\item git\index{Git}. I use version 2.34.1.
\end{itemize}

All of this can be installed on an Ubuntu Linux system using the usual \emph{apt} installation
...
Additionally, the server must be able to handle outgoing mail. Bible OL will send mail to users, and
there must be a way for the server to handle this. There are several ways to configure this, and it
probably requires information from a mail service provider. The details are therefore beyond the
...
Emdros version 3.8.0 can be downloaded from \url{https://emdros.org/download.html}. After
downloading a tar file from this location, unpack it and make a copy of the \texttt{INSTALL} file.
Unfortunately, the \texttt{INSTALL} file is overwritten during the build process, so you will need a
copy of it.

There are several ways to build Emdros. The simplest for an Ubuntu system is to follow the
instructions under the heading ``Building a .deb from an Emdros tarball'' in the \texttt{INSTALL}
file. Briefly, the steps involved are:
...
\emph{Note:} If you have forked\index{fork GitHub} Bible OL on GitHub you should replace the URL in the ``git''
...
\$config['users\_per\_page']\index{users per page} & The number of users to display on each page when
displaying the list of all users.\\

\$config['exams\_per\_page']\index{exams per page}\index{exam} & The number of exams to display on each page when
displaying the list of all exams.\\

\$config['lines\_per\_page']\index{translations!lines per page} & The number of items to display on  each page when
...
Replace `https://example.com' with the top URL of the website for your Bible OL installation. NOTE:
It is highly recommended that your installation runs HTTPS\index{HTTPS}. If it doesn't, you should
change \texttt{\$config['cookie\_secure']} to \texttt{false}; but then some parts of the system may
not work reliably.
...
\begin{lstlisting}[language=bash]
php index.php users generate_administrator %\textrm{\textit{username}}% %\textrm{\textit{first-name}}% %\textrm{\textit{last-name}}% %\textrm{\textit{password}}%
\end{lstlisting}
...
If you need to use any other character in the name or password, you can change it using the web
interface once Bible OL is up and running.
...
Briefly, setting up a website involves creating a file named, for example, ``bibleol.conf'' in
\texttt{/etc/apache2/sites-available} with this contents:

\begin{lstlisting}
<VirtualHost *:80>
    ServerName %\textrm{\emph{website name}}%
    DocumentRoot %\textrm{\emph{installation directory}}%
    ErrorLog ${APACHE_LOG_DIR}/bibleol-error.log
    CustomLog ${APACHE_LOG_DIR}/bible-access.log combined
 
    <Directory %\textrm{\emph{installation directory}}%>
        AllowOverride All
    </Directory>
 
    <Directorymatch "%\textrm{\^{}}%/.*/\.git/">
        Order deny,allow
        Deny from all
    </Directorymatch>
</VirtualHost>
...
This will enable HTTP\index{HTTP} access to the website.

\subsubsection*{Substep 2}

Enable the ``rewrite'' and (if you are using HTTPS\index{HTTPS}) the ``ssl'' module in Apache. This can be done
...
In Bible OL installation directory, copy the file \texttt{.htaccess-dist} to \texttt{.htaccess} and
modify the following line in the copy:
...
Enable HTTPS\index{HTTPS} access. Although this is not a requirement, it is highly recommended.

There are various ways to do this. One possibility is to use the ``certbot''\index{certbot} program
from \url{https://certbot.eff.org}. The following commands can be used:
...
This step involves making changes to the PHP configuration file. That file is found at
\texttt{/etc/php/8.1/apache2/php.ini} (where 8.1 should be replaced by your PHP version number).
...
Enable upload of files of at least 10 MB: In the PHP configuration file, ensure that the
configuration variables have at least these values:
...
\emph{session.gc\_probability} and \emph{session.gc\_divisor} in the PHP configuration file. You
must ensure that the configuration variables have these values:
...
\subsection{Step 11. Cron Jobs}\index{cron}
...
\begin{lstlisting}
cd %\textrm{\emph{installation directory}}%; php index.php users expire_users
\end{lstlisting}

For details see Section \ref{sec-account-expiry}.

\vspace{2mm}
2) If your system is interfacing to the resources website\index{resources website|hyperit} (see
Section \ref{sec-resources-web}) the following command should run once an hour:

\begin{lstlisting}
cd %\textrm{\emph{installation directory}}%; php index.php pic2db > /dev/null
\end{lstlisting}

And, optionally, the following command should run once a week:

\begin{lstlisting}
cd %\textrm{\emph{installation directory}}%; echo Checking URLs; php index.php urls check_urls; echo Check finished
\end{lstlisting}
...
\begin{itemize}
\item SQLite3\index{SQLite3} (database system used by Emdros). I use version 3.37.2.
\item PHP\index{PHP}. The current main installation on \texttt{https://learner.bible} uses PHP
  version 8.1, so it is probably a good idea to avoid using PHP features added since that version.
\item git\index{Git}. I use version 2.34.1.
\item The GNU \emph{make}\index{make} program. I use version 4.3.
\end{itemize}

All of this can be installed on an Ubuntu system using the usual \emph{apt} installation
program. I have not done a thorough investigation into the minimum required versions of these
software packages; you should not see the versions mentioned above as minimum requirements.

You will also need to install the Emdros database system, a Less compiler, and a TypeScript
compiler. This is detailed in the following subsections.
...
\subsection{Installing Lessc\index{lessc!installation} and Tsc\index{tsc!installation}\index{TypeScript!compiler|see {tsc}}}\label{sec-installing-lessc-tsc}

\emph{Lessc}\index{lessc} is the Less\index{Less} compiler, \emph{tsc}\index{tsc} is the
TypeScript\index{TypeScript} compiler. Both of these run under \emph{nodejs},\index{nodejs} which is a
stand-alone JavaScript\index{JavaScript} runtime system.

For information about how to use these two programs, see \url{https://lesscss.org} and
\url{https://www.typescriptlang.org}, respectively.

\emph{Nodejs} comes with its own software package manager, \emph{npm.} Unfortunately, \emph{npm} and
Ubuntu's package manager, \emph{apt,} don't always play well together. On an Ubuntu system, I will
therefore recommend using the following commands to install the software:
...
An alternative to installing \emph{node-less} and \emph{node-typescript} in this manner would be to
use \emph{npm} to install them.\footnote{You can use the commands ``sudo npm install -g less'' and
  ``sudo npm install -g typescript''.} This may give you a newer version of \emph{lessc} and
\emph{tsc,} but they may be incompatible with the version of \emph{nodejs} on your system.
...
MQL is a language for manipulating Emdros\index{Emdros}\footnote{\url{https://emdros.org}.} text
...
VirtualKeyboard.\allowbreak{}full.3.7.2\index{virtualkeyboard} & A JavaScript-based virtual keyboard%
...
\begin{center}
  \includegraphics[width=0.8\textwidth]{gen1_29-30.png}
\end{center}
...
\begin{my-tabu}{cll}{ \headiii{Character}{Greek accent name}{English accent name} }
    {\LARGE ά} & Oxia\index{oxia} (οξεία) & Acute\index{accent!acute}\index{acute accent|see {accent, acute}}\\
    {\LARGE ὰ} & Varia\index{varia} (βαρεία) & Grave\index{accent!grave}\index{grave accent|see {accent, grave}}\\
    {\LARGE ᾶ} & Perispomeni\index{perispomeni} (περισπωμένη) & Circumflex\index{accent!circumflex}\index{circumflex accent|see {accent, circumflex}}\\
\end{my-tabu}
...
\chapter{Database Description Files}\index{Database Description File}\index{Description File|see {Database Description File}}\label{chap-db-desc-files}
...
The first two files are JSON\index{JSON} files. A JSON file contains key/value pairs%
...
\emph{make}\index{make} program and the \emph{Makefile} in the top source directory, which will
generate the corresponding ugly JSON files.
...
\begin{my-longtabu}{lX}{ \headii{Key}{Value} }
  ...
  picDb & The URL of the resources website\index{resources website} (see Section
  \ref{sec-resources-web}). This value may be \emph{null.}\\
...
selection box\index{grammar selection box}, which is shown if you click ``MyView'', the other is in the
...
\xml{randomize} & A Boolean value indicating if questions should be given in a random order. If this
XML element is absent, it is assumed have the value \emph{true}.\\
...
enumeration\index{Emdros!enumeration} type, the request feature is displayed as a multiple choice question:
...
\emph{objectSettings}\index{objectSettings} of the Database Specification file (see Section
...
If the SQL statement yields only a single value, it is not presented as a multiple choice selector
as with this word:

\begin{center}
  \includegraphics[width=0.3\textwidth]{pronsuf2.png}  % From Isaiah 60:3
\end{center}
...
\textbf{Read this chapter if you need to understand or modify the way Bible OL provides hints for
  some words.} \plainbreak{3}


In some cases, a Hebrew or Greek word form may have several interpretations depending on the context. For
...
A separate ``Hints Database''\index{Hints Database|hyperit} exists for Hebrew and Greek with the
information necessary for this to work.

The Hints Databases are SQLite3\index{SQLite3} databases containing all possible values for the
relevant feature. For a detailed description of the Hints Database see Appendixes
\ref{app-hintsdb-heb} and \ref{app-hintsdb-gr}

The \emph{hint} feature is implemented as a pseudofeature (see Section \ref{sec-pseudofeature}).
Hints are only available for some word classes, and only in situations where the word form is ambiguous.
...
server with hostname \texttt{learner.bible}, you can use this URL:
\url{https://learner.bible/text/show_text/ETCBC4-translit/Genesis/1/2/5}.
...
\begin{my-longtabu}{lX}{ \headii{Variable}{Contents} }
  ...
  (For example, \texttt{https://learner.bible/}.)\\
  ...
  l\_icon\_map\hmmindex{l_icon_map JavaScript variable@\string\emph{l\_icon\_map} JavaScript variable} &
  This variable translates internal icon names to HTML class names used to display the icons. This
  translation originates in the file \texttt{myapp/helpers/icon\_helper.php}.\\
...
\begin{lstlisting}[frame=]
https://%\textrm{\textit{hostname}}%/text/show_quiz?quiz=%\textrm{\textit{excercisename}}%&count=%\textrm{\textit{numberOfQuestions}}%
https://%\textrm{\textit{hostname}}%/text/show_quiz_univ?quiz=%\textrm{\textit{excercisename}}%&count=%\textrm{\textit{numberOfQuestions}}%
\end{lstlisting}
...
\texttt{Nestle~1904/demo/case.3et} on the server with hostname \texttt{learner.bible}, you can
use this URL:
\url{https://learner.bible/text/show_quiz?quiz=Nestle\%201904/demo/case.3et&count=10}. This
...
\begin{my-longtabu}{lX}{ \headii{Variable}{Contents} }
  ...
  l\_icon\_map & The same as in Section \ref{displaying-text}.\\
\end{my-longtabu}
...
%%%%%%%%%%%%%%%%%%%% Running an Exercise %%%%%%%%%%%%%%%%%%%%
\section{Setting Exercise Timers}
Teachers can also set time limits for their exercises. The time limit will only apply to the entirety 
of the exercise, and not individual questions. For example if the exercise limit is for five minutes, 
and there are ten questions, the student can use the five minutes however they choose.
Once the timer runs out, the exercise will be submitted with whatever data that was present.

\begin{figure}[h]
  \includegraphics[width=\textwidth]{exercise_timer.png}
  \caption{The exercise timer tab}
\end{figure}
...
pics & An array of integers identifying pictures on the resources website\index{resources website} (see Section
\ref{sec-resources-web}) that are relevant for this word. The first three values in the array are the book
number, chapter number, and verse number. The remaining elements are IDs of pictures on the resources
website.\\
...
\begin{tabu*}{@{}lX@{}}
  ...
  \midrule
    ...
    \index{virtual keyboard} to facilitate the typing of text in a foreign alphabet. Note: This
...
exams & Manages exams\index{exam}.\\
...
It is used to retrieve information from the resources website\index{resources website} (see Section
\ref{sec-resources-web}).\\
...
Mod\_exams & Manages exams\index{exam}.\\
...
libraries/picdb.php & Class for retrieving picture references and URL references from the resources
website\index{resources website} (see Section \ref{sec-resources-web}).\\
...
that runs at \url{https://learner.bible}.
...
redirect\_uri  & \texttt{'https://learner.bible/oauth2/google\_callback'}\\
...
client\_id     & Our Facebook app ID\index{Facebook!app ID}, configured in the file
                 \texttt{myapp/\allowbreak{}config/\allowbreak{}ol.php}\index{ol.php}.\\
redirect\_uri  & \texttt{'https://learner.bible/oauth2/facebook\_callback'}\\
scope          & \texttt{'email'}\\
state          & A random value, stored in CodeIgniter's session mechanism.\\
\end{my-tabu}
...
\texttt{https://learner.bible/oauth2/*\_callback} with the following GET parameters:
...
\section{Account Expiry}\label{sec-account-expiry}
...
The \emph{bible\_refs} table contains links between Bible verses and pictures on the resources
website\index{resources website} (see Section \ref{sec-resources-web}) that are relevant for the
...
picture    & Integer & The number of a picture on the resources website.\\
\end{my-longtabu}
...
The \emph{bible\_urls} table contains links between Bible verses and URLs.\footnote{Currently, the
  URLs are configured on the resources website\index{resources website} (see Section
  \ref{sec-resources-web}), but they could come from other sources.} The table contains these fields:
...
\section{The \emph{exam}, \emph{exam\_active}, \emph{exam\_finished}, \emph{exam\_results} and
  \emph{exam\_status} Tables}\index{exam table}\index{exam\_active table}\index{exam\_finished table}\index{exam\_results table}\index{exam\_status table}
...
\begin{my-longtabu}{>{\itshape}llX}{ \headiii{\textup{Column}}{Type}{Contents} }
id                  & Integer & Unique number identifying this entry.     \\
strongs             & Integer & The Strong's number\index{Strongs's number} for the lexeme. \\
...
Section \ref{sec-installing-lessc-tsc} describes how to install the Less compiler
\emph{lessc}\index{lessc}. Details of the Less language can be found at \url{https://lesscss.org}.
...
Makefile\index{Makefile} in the top directory. The command ``make\index{make} styles/ol.css
...
Section \ref{sec-installing-lessc-tsc} describes how to install the TypeScript compiler \emph{tsc}.\index{tsc}
...
simply ``make'')\index{make} will compile the TypeScript files (plus the Less file and some Database Description Files (see Chapter \ref{chap-db-desc-files})).
...
\begin{center}
  \parbox{0.8\textwidth}{
    \includegraphics[width=0.8\textwidth]{clause2.png}  % Luke 1:5
...
This ensures that URLs such as, for example, \texttt{'https://learner.bible/xxx/XXX\_foobar} is
...
A few additional websites complement the function of Bible OL: The resources website and the SHEBANQ
website.



%%%%%%%%%%%%%%%%%%%% The Resources Website %%%%%%%%%%%%%%%%%%%%
\section{The Resources Website}\label{sec-resources-web}\index{resources website|hyperit}

The resources website is a collection of photos from the Middle East. Many of them relate to events
and places described in the Bible. The photos have descriptive texts that contain Bible references.
The URL of the resources website is \url{https://resources.learner.bible}.

Bible OL can use information from the resources website to add picture links to Bible passages. If a
photo in the resources website refers to, for example, \bibleref{Exodus}{3}{2}, and a user ticks the
...
\noindent
in a cron\index{cron job} job. This command calls Bible OL's \emph{Ctrl\_pic2db} controller (in the
file \texttt{myapp/\allowbreak{}controllers/\allowbreak{}Ctrl\_pic2db.php}), which requests
information from the resources database.

By accessing the URL \url{https://resources.learner.bible/jsonrefs.php}, the \emph{Ctrl\_pic2db}
...
In addition to the pictures, the resources website may also provide URLs associated with various
Bible verses. These URLs are configured in the resources website but are otherwise unrelated to the
...
\url{https://resources.learner.bible/jsonurls.php}, the \emph{Ctrl\_pic2db} controller receives a JSON
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%% Docker Development %%%%%%%%%%%%%%%%%%%%
\chapter{Docker Development}

\textbf{Read this chapter if you want to.}
\plainbreak{3}

Run BibleOL in a Docker container.

%%%%%%%%%%%%%%%%%%%% Docker Context %%%%%%%%%%%%%%%%%%%%
\section{Docker Context and Terms}

A Docker container is a software package containing all of the dependencies required for an application. BibleOL has many important packages that it relies on, and the exact versions of these packages can be hard to track. This motivated our development team to create a container for BibleOL, so that it is more accessible for everyone. We have made our containers available for Mac, Windows, and Linux users.

\begin{my-longtabu-nomid}{lX}{ \headii{Term}{Meaning} }

\parbox[t]{3cm}{Docker-Container\index{question|hyperit}}& a lightweight, standalone, 
and executable software package that includes everything needed to run an 
application, such as the code, runtime, system tools, libraries, and settings. 
It is a running instance of a Docker image.\\
\midrule

\parbox[t]{3cm}{Docker-Image\index{question|hyperit}}& A Docker image is a read-only template that contains the instructions for creating a Docker container. It includes the application code, libraries, dependencies, and configuration files needed to run the software. Images are used to instantiate containers and can be shared via registries like Docker Hub.\\
\midrule
\parbox[t]{3cm}{Docker-Hub\index{question|hyperit}}& Docker Hub is a cloud-based registry service for storing, sharing, and distributing Docker images. It allows users to publish images publicly or privately, collaborate, and pull images for container deployment.\\
\midrule
\end{my-longtabu-nomid}

%%%%%%%%%%%%%%%%%%%% Installing BibleOL with Docker %%%%%%%%%%%%%%%%%%%%
\section{Installing BibleOL with Docker}

\emph{All of the instructions and code for installing BibleOL can be found in \textbf{BibleOL/Dockerfiles}}\\

There are two primary docker images for the Bible Online Learner. The image bibleol-arm is used for Mac operating systems that have an M1 or M2 chip, this is most of the newer models. The other image bibleol-amd will work on all other architectures including Windows, Linux, and Mac (Intel). The initial installation will take approximately 15 minutes, if the image is not already downloaded to your machine. Subsequent installations will generally take between 1-2 minutes. There is no need to be fluent in Docker commands to build these projects, because each sub-directory contains shell scripts and Makefiles that automate the installation.

\subsection{Apple Silicion Installation for M1 and M2 Chips}

\textbf{Installation Steps: }
\begin{enumerate}
  \item Open a terminal and change your directory to BibleOL/Dockerfiles/bibleol-arm.
  \item Build the project by typing 'make'. If you do not have make you can install it with apt-get install make.
  \item Go to localhost:8000 and you should be viewing the home page.
  \item Login as the default administrator with the following credentials
  \begin{itemize}
    \item username: admin
    \item password: bibleol\_pwd
  \end{itemize}
\end{enumerate}

\subsection{Linux/Windows/Mac(Intel) Installation}
\textbf{Installation Steps: }
\begin{enumerate}
  \item Open a terminal and change your directory to BibleOL/Dockerfiles/bibleol-amd.
  \item Build the project by typing 'make'. If you do not have make you can install it with apt-get install make.
  \item Go to localhost:8000 and you should be viewing the home page.
  \item Login as the default administrator with the following credentials
  \begin{itemize}
...
\textbf{Commands: }
\begin{itemize}
  \item Type make stop to stop or pause the docker container
  \item Type make start to start a stopped container
  \item Type make clean to delete the image and container
\end{itemize}
...
\chapter{ETCBC4 Hints Database}\label{app-hintsdb-heb}\index{Hints Database}
...
string in the form of combinations of \emph{``feature=value''} or \emph{``feature≠value'',} where
...
The Hints Database is generated by the programs available at
\url{https://github.com/EzerIT/ETCBC4BibleOL}. The data was collected by Dr Oliver Glanz\index{Glanz,
  Oliver} of Andrews University.
...
The \emph{strongs} feature is an integer feature containing Strong's number\index{Strongs's number} for the lemma. The
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%% Appendix: Nestle1904 Hints Database %%%%%%%%%%%%%%%%%%%%
\chapter{Nestle1904 Hints Database}\label{app-hintsdb-gr}\index{Hints Database}

The Hints Database (see Chapter \ref{chap-hints}) for the Nestle1904 Emdros database has the name
\texttt{nestle1904\_hints.db}. Its structure is defined by this SQL statement:
...
The \emph{self} column contains a monad\index{monad} (see Chapter \ref{chap-emdros-use}) which
identifies the ambiguous word within the Nestle1904 database. The \emph{hint} column contains the hint
string in the form of combinations of \emph{``feature=value''} or \emph{``feature≠value'',} where
\emph{feature} and \emph{value} must be localized before they are displayed to the user.


%%%%%%%%%%%%%%%%%%%% The Origin of the Nestle1904 Hints Database %%%%%%%%%%%%%%%%%%%%
\section{The Origin of the Nestle1904 Hints Database}\index{Hints Database}

The Hints Database is generated by the programs available at
\url{https://github.com/EzerIT/nestle1904}. The data was collected by Dr Oliver Glanz\index{Glanz,
  Oliver} of Andrews University.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%% Appendix: Lexicon Files %%%%%%%%%%%%%%%%%%%%
\chapter{Lexicon Files}\label{app-lex}\index{lexicon|hyperit}\index{lexicon files|hyperit}
...
% I'm using the DejaVu Sans Mono font because it supports IPA characters.
...
\begin{my-tab}{ll|ll}{ \headiv{Heading}{Contents}{Heading}{Contents} }
...
% I'm using the DejaVu Sans Mono font because it supports Greek characters.
...
\begin{my-tab}{ll}{ \headii{Heading}{Contents} }
...
% I'm using the DejaVu Sans Mono font because I've done it above
