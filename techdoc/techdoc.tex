...
License\footnote{\url{http://opensource.org/licenses/MIT}}. The code is Copyright © 2016 by Claus
...
When you compile Emdros, be sure to include support for PHP\index{PHP}. I use this
...
If you want to include support for Java\index{Java}, Python\index{Python}, or Perl\index{Perl}, you
...
You can download Bible OL from GitHub\index{GitHub} using this command:
...
\subsection{Step 4. Configure MySQL\index{MySQL}}
...
\subsection{Step 6. Add Administrator\index{administrator}}
...
\subsection{Step 7. Apache Configuration}\index{Apache!Configuration}
...
CodeIgniter      \index{CodeIgniter}     & EzerIT/CodeIgniter         & bcit-ci/CodeIgniter \\
...
a stand-alone JavaScript\index{JavaScript} runtime system. For information about how to install and use the Less
...
Bible OL. Currently, the Makefile\index{Makefile} that comes with Bible OL checks that the compiler version is
...
\emph{Bootstrap.}\footnote{\url{http://getbootstrap.com}.}
...
\begin{itemize}
\item The user database\index{user database}, which contains information about registered users.
...
\item When the user click on the ``Finish''\index{finish button@``Finish'' button} button, the
...
bootstrap\index{Bootstrap} & A JavaScript framework used on the client side of Bible OL.

Webpage: \url{http://getbootstrap.com}.
...
CodeIgniter\index{CodeIgniter} & The PHP framework used by the server side of Bible OL.
...
js & JavaScript\index{JavaScript} files from various sources. The files \texttt{ol.js}, \texttt{editquiz.js}, and
...
SILfonts & A collection of Greek and phonetic fonts\index{font}. These files are not used directly
...
\item Hebrew/Aramaic characters in \emph{ETCBC4 transcription}.%
  \index{alphabet!transcribed|hyperit}\index{transcribed alphabet|see {alphabet, transcribed}}
...
\emph{continuation}\index{continuation|hyperit} with each word. The continuation feature contains
...
\begin{my-tabu}{cc}{ \headii{Text}{Continuation} }
...
\emph{continuation}\index{continuation} feature (see Section \ref{sec-contin}).
...
α\hspace{-0.5mm}̍\hspace{0.5mm}.\footnote{Source:
  \url{http://www.opoudjis.net/unicode/unicode_gkbkgd.html}, accessed 4 May 2016.}
...
.LANG.prop.json &
Database Localization File\index{Database Localization File}\index{Localization File|see {Database Localization File}} &
\ref{sec-db-local}\\
...
\begin{my-longtabu}{lX}{ \headii{Key}{Value} }
  ...
  propertiesName\label{propname} & The primary name of the Database Localization
  Files\index{Database Localization File} (see Section \ref{sec-db-local}).\\
  ...
  (see Section \ref{sec-visual}). \\

  objHasSurface & The name of the Emdros object type that contains the \emph{surfaceFeature.}
  (Typically, ``word''.)\\

  suffixFeature & The name of the feature that contains the continuation\index{continuation} for the
  word to display (see Section \ref{sec-contin}). For ETCBC4 using the native alphabet this value is
  ``continuation\_utf8'', for ETCBC4 using the transliterated alphabet the value is
  ``continuation\_translit'', for nestle1904 the value is \emph{null}. \\
  ...
  useSofPasuq & A Boolean value indicating if a \emph{sof pasuq}\index{sof pasuq} (\heb{׃}) should
...
If the feature is a \emph{pseudofeature}\index{pseudofeature}, additional fields are present in the
GrammarFeature structure. See Section \ref{sec-pseudofeature} for more information.
...
the primary name\index{primary name} of the Database Localization File%
\index{Database Localization File}
(see Section \ref{sec-db-local}) for the superset.
...
\subsection{Pseudofeatures}\label{sec-pseudofeature}\index{pseudofeature|hyperit}
...
Details about psuedofeatures are found in the \emph{Grammarfeature} for the relevant feature. If the
feature is a pseudofeature, its GrammarFeature has the format exemplified in Listing
\ref{list-gfps-syntax}.

\begin{lstlisting}[escapechar=\#,caption=GrammarFeature syntax for a pseudofeature,label=list-gfps-syntax]
    {
        "mytype": "GrammarFeature",
        "name": "english",
        "indirdb": "glossdb_hebrew.db",
        "sql": [ "english", "heb_en", "lex='%s' AND vs='%s' AND language='%s'" ],
        "sqlargs": [ "lex", "vs", "language" ],
        "multiple": false
    }
\end{lstlisting}

The \emph{mytype} and \emph{name} values are common to all GrammarFeatures (see Section
\ref{sec-grammarfeature}). The presence of the remaining values identify the feature as
pseudofeature.
...
\item \emph{sql} contains three values: The database field, database table, and database search
  string used to find the pseudofeature within the database.
\item \emph{sqlargs} contains the names of features used to build the database search string.
\item \emph{multiple} is true if the datbase may contain more than one entry for a given pseudofeature.
\end{itemize}

Taking the pseudofeature \emph{english} from the above listing as an example, we see that the
feature is found in an SQLite3 database file called \texttt{db/glossdb\_hebrew.db}.

The value of the pseudofeature is found by executing the SQL query

\begin{lstlisting}[language=SQL]
SELECT english FROM heb_en WHERE lex='...' AND vs='...' AND language='...';
\end{lstlisting}

\noindent
where the dots are replaced by the values of the features \emph{lex, vs,} and
\emph{language,} respectively.
...
\section{Database Localization Files: PRIM.LANG.prop.json}\label{sec-db-local}\index{Database Localization File|hyperit}
...
Its structure is described in TypeScript as the \emph{Localization} interface%
...
\begin{my-longtabu}{lX}{ \headii{Key}{Value} }
  ...
  dbcopyright & An HTML string containing copyright\index{copyright} information for the database.\\
...
the English localization of the \emph{nestle1904} database (from the file
\texttt{db/nestle1904.en.prop.json}).
...
\section{Database Type Information Files: PRIM.typeinfo.json}\label{sec-tif}\index{Database Type Information File|hyperit}
...
\section{Database Book Order Files: PRIM.bookorder}\label{sec-bookorder}\index{Database Book Order File|hyperit}
...
\xml{properties} & The primary name\index{primary name} of the Database Localization
File\index{Database Localization File}.\\
...
\begin{my-longtabu}{lX}{ \headii{Variable}{Contents} }
  ...
  A JavaScript object whose value is the contents of the Database Localization File
...
the server takes place before the user presses the ``Finish'' button%
\index{finish button@``Finish'' button}.
What happens when the ``Finish'' button is pressed, depends on whether the user is logged
in or not. If the user is logged in, the client sends the result to the server URL
\texttt{http://}\textit{hostname}\texttt{/statistics/update\_stat}, which updates the
statistics\index{statistics} for the user; if the user not logged in, the client instructs the
server to display the \emph{Select a Quiz} web page.
...
A \emph{MultipleMonadObject}%
\index{MultipleMonadObject TypeScript interface@\emph{MultipleMonadObject} TypeScript interface}
has no fields other than those defined for \emph{MonadObject}.
...
large library for performing a number of tasks.
...
\section{PHP Parameter Type Hinting}\label{php-typing}\index{PHP!type hinting}
...
The user database is a MySQL database that contains information about the users and classes
...
alphabet         & font                 & sta\_quiztemplate   \\
bible\_refs      & heb\_urls            & sta\_requestfeature \\
bible\_urls      & migrations           & sta\_universe       \\
class            & personal\_font       & user                \\
classexercise    & sta\_displayfeature  & userclass           \\
exercisedir      & sta\_question        & userconfig          \\
exerciseowner    & sta\_quiz            &                     \\
\end{tabu}
\end{center}
...
the user ID provided by Google. Their \emph{oauth2\_login} field is set to \emph{`google'} (1).\index{login!Google}

Facebook users have usernames such as ``fcb\_10206545794576996'', where 10206545794576996 is
the user ID provided by Facebook. Their \emph{oauth2\_login} field is set to \emph{`facebook'} (1).\index{login!Facebook}
...
simply ``make'')\index{make} will compile the TypeScript files (and the Less file). The Makefile checks that the
...
\section{Internationalization of the Server Code}

This section deals with creating an internationalized and localized version of the PHP server code.
This uses the mechanism provided by CodeIgniter.

As an example, let us assume that we want the server to display the text ``Roses are red''. In a
...
To internationalize this code, three steps are necessary:

First, we must first load a relevant localization file. For example, thus:

\begin{lstlisting}[language=PHP]
$this->lang->load('flowers', $language);
\end{lstlisting}

This will load the file \texttt{myapp/language/XX/flowers\_lang.php} where \texttt{XX} is replaced by
the specified language.

Second, we must add the English text to \texttt{myapp/language/english/flowers\_lang.php} thus:

\begin{lstlisting}[language=PHP]
$lang['rose_color'] = 'Roses are red';
\end{lstlisting}

The string \lstinline|'rose_color'| can be anything, as long as it is unique.

Other localization information can be added as well. For example, a German translation can be
written in \texttt{myapp/language/de/flowers\_lang.php}:

\begin{lstlisting}[language=PHP]
$lang['rose_color'] = 'Rosen sind rot';
\end{lstlisting}

Third, the original PHP code must be replaced by:

\begin{lstlisting}[language=PHP]
echo $this->lang->line('rose_color');
\end{lstlisting}

Depending on the language specified in the \emph{\$this->lang->load} call above, the \emph{echo}
statement will output ``Roses are red'' in the appropriate language.
...
All localization information for the client is found in the server file
\texttt{myapp/\allowbreak{}language/\allowbreak{}XX/\allowbreak{}js\_lang.php} where \texttt{XX} is
replaced by the specified language. Before passing control to the client, the server reads the
contents of this file and stores it as key/value pairs in the JavaScript variable
\emph{l10n\_js}\index{l10n_js JavaScript variable@\emph{l10n\_js} JavaScript variable|hyperit}.

To internationalize the above client code, two steps are necessary:

First, \emph{in the server code} we must add the English text to
\texttt{myapp/language/english/js\_lang.php} thus:

\begin{lstlisting}[language=PHP]
$lang['elephant_size'] = 'Elephants are big';
\end{lstlisting}

The string \lstinline|'elephant_size'| can be anything, as long as it is unique.

Other localization information can be added as well. For example, a Danish translation can be
written in \texttt{myapp/language/da/js\_lang.php}:

\begin{lstlisting}[language=PHP]
$lang['elephant_size'] = 'Elefanter er store';
\end{lstlisting}

Second, the original TypeScript code must be replaced by:
...
\section{Internationalization of the Databases}

In addition to providing localized versions of the server and client strings, a translation must
also provide localized versions of all the Database Localization Files. See Section \ref{sec-db-local}.
...
performance. Currently, the Learning Journey URL is \url{http://statdb.3bmoodle.dk}.

Learning Journey directly accesses the statistics and user tables of the Bible OL user
database.\index{user database} (See Chapter \ref{chap-user-database}.)
...
The \emph{text} features work together with the \emph{continuation}\index{continuation} features as
described in Section \ref{sec-contin}. The \emph{continuation} features exist in these variants:
...
The \emph{g\_word} features are very similar to the \emph{text} features. The \emph{g\_word}
features are not used by Bible OL because their information is available by other means. The
difference between \emph{g\_word} and \emph{text} is that \emph{g\_word} does not use the proper
final form of the Hebrew consonants, and it includes punctuation information that follows a word.
This punctuation information is typically the character \heb{׃} \emph{(sof pasuq)}\index{sof pasuq}
or \heb{־} \emph{(maqaf),}\index{maqaf} but it may also include additional characters.
...
The \emph{suffix} features contain characters that follow a word (as described above for
\emph{g\_word}). Possible suffixes are:
...
The following word features were generated by my program
\emph{emdros\_updater}\index{emdros\_updater} which is currently not published or documented:

\begin{center}
  \begin{tabular}{lll}
    continuation           & g\_uvf\_cons\_utf8  &  suffix\_utf8                  \\
    continuation\_translit & g\_uvf\_translit    &  text                          \\
    continuation\_utf8     & g\_vbe\_cons\_utf8  &  text\_cons\_utf8              \\
    frequency\_rank        & g\_vbe\_translit    &  text\_nocant\_utf8            \\
    g\_lex\_cons\_utf8     & g\_vbs\_cons\_utf8  &  text\_nopunct\_translit       \\
    g\_nme\_cons\_utf8     & g\_vbs\_translit    &  text\_translit                \\
    g\_nme\_translit       & g\_word\_cons\_utf8 &  text\_utf8                    \\
    g\_pfm\_cons\_utf8     & lex\_cons\_utf8     &  verb\_class                   \\
    g\_pfm\_translit       & lexeme\_occurrences &  vocalized\_lexeme             \\
    g\_prs\_cons\_utf8     & suffix              &  vocalized\_lexeme\_cons\_utf8 \\
    g\_prs\_translit       & suffix\_gender      &  vocalized\_lexeme\_translit   \\
    g\_qere                & suffix\_number      &  vocalized\_lexeme\_utf8       \\
    g\_qere\_translit      & suffix\_person      &   \\
    g\_qere\_utf8          & suffix\_translit    &   \\
  \end{tabular}
\end{center}
...
The current Word Database was originally generated for the old WIVU\index{WIVU} Emdros database. It
