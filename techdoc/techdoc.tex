...
\item MySQL\index{MySQL} (database system). I use version 5.7.13.
\item SQLite3\index{SQLite3} (database system used by Emdros). I use version 3.11.0.
\item PHP\index{PHP}. I use version 5.6. \emph{Note: Version 7.0 and later does not work!}
...
directory \texttt{/etc/php/5.6/apache2/conf.d/}:
...
\$config['site\_url']\index{site URL|see {URL, site}}\index{URL!site} & The URL used to access the site.\\
...
If you plan to use \emph{Learning Journey} (see Chapter \ref{chap-lj}, set \texttt{\$config['lj\_enabled']} to \texttt{true}.
...
\item When the user clicks on the ``GRADE task''\index{grade task button@``GRADE task''
  button}\index{finish button@``Finish'' button|see {``GRADE task'' button}} or ``SAVE
...
\begin{lstlisting}[escapechar=\#,caption=Featuresetting syntax for a pseudofeature,label=list-ps-syntax]
    "english": {
        ...
        "matchregexp": "\/^(.+[;,] +)?(\\(.*\\) *)?{0}( *\\(.*\\))?([,;].+)?$\/i",
        "indirdb": "mysql",
        "sql_command": "SELECT gloss FROM {PRE}lexicon_heb heb JOIN {PRE}lexicon_heb_en en ON en.lex_id=heb.id WHERE lex='%s' AND vs='%s' AND language='%s'",
...
\item \emph{sql\_command} is the SQL command required to retrieve the value of the feature. A
  ...
  \texttt{\{PRE\}} with the correct database table prefix. The string \texttt{\%d} or \texttt{\%s}
  indicates the position of the parameters required for the request. These parameters are specified
  under the key \emph{sqlargs}.
\item \emph{sqlargs} is an array of features whose values are to be used in place of the
  \texttt{\%d} or \texttt{\%s} strings in \emph{sql\_command}.
...
If we assume that the database table prefix\footnote{The database table prefix can be set in the
  file \texttt{myapp/config/database.php} (See Section \ref{sec-database-php}).} is ``bol\_'',
the value of the pseudofeature is found by executing the SQL query

\begin{lstlisting}[language=SQL]
SELECT gloss FROM bol_lexicon_heb heb JOIN bol_lexicon_heb_en en ON en.lex_id=heb.id
    WHERE lex='...' AND vs='...' AND language='...'
\end{lstlisting}
...
the server takes place before the user presses the ``GRADE task''\index{grade task button@``GRADE
  task'' button} or ``SAVE outcome''\index{save outcome button@``SAVE outcome'' button} button. What
...
Learning Journey (see Chapter \ref{chap-lj}).
...
alphabet      &  heb\_urls            &  sta\_displayfeature \\
bible\_refs   &  language\_comment    &  sta\_question       \\
bible\_urls   &  language\_LANG       &  sta\_quiz           \\
class         &  lexicon\_greek       &  sta\_quiztemplate   \\
classexercise &  lexicon\_greek\_LANG &  sta\_requestfeature \\
db\_localize  &  lexicon\_heb         &  sta\_universe       \\
exercisedir   &  lexicon\_heb\_LANG   &  user                \\
exerciseowner &  migrations           &  userclass           \\
font          &  personal\_font       &  userconfig          \\
\end{tabu}
\end{center}
...
TODO: Here are copies of text from old versions:
...
\begin{itemize}
\item Explain the function of the installed git-hook.
\item Update \texttt{bolsetup.sql} to include translator privileges and translation files.
\item Plug-ins.
\item Learning Journey.
\item In the code, switch from using \texttt{\$config['site\_url']} to the already existant
  \texttt{\$config['base\_url']} (with an optional final '/' removed). Also remove
  \texttt{\$config['site\_url']} from this documentation.
\item Search this document for ``Database Localization File''.
\item Move everything between ``TODO: MOVE THIS - START'' and ``TODO: MOVE THIS - END'' to a section
  on localization.
\item Describe the ``.pretty.json'' files in \texttt{db}.
\item \texttt{php index.php maketypeinfo index \textit{databasename}} doesn't work.
