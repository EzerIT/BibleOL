...
\newcommand*{\hypref}[1]{\hyperref[#1]{\ref*{#1}}}  %% Delete this?
\newcommand{\forlater}[1]{} %Omit text
\newcommand*{\xml}[1]{\texttt{<#1>}}
\newcommand*{\xmla}[1]{\texttt{#1}} % An xml attribute


% Write a term and index it
\newcommand*{\indexw}[1]{%
  #1%
  \index{#1}%
}

 % Write a term and index a lower case version of it
\newcommand*{\indexl}[1]{%
  #1%
  \lowercase{\def\temp{#1}}%  \lowercase{\def\temp{ABC}} becomes \def\temp{abc}
...
\parbox[t]{3cm}{\indexw{quiz}\par \indexw{exercise}} & These terms are synonymous. They refer to an exercise executed
by a user.\\

\midrule

\indexw{quiz template}\index{template} & A file located in the \texttt{quizzes} directory. In XML form it describes how Bible
OL should generate questions for an exercise.\\

\midrule

\parbox[t]{3cm}{\indexw{question}\par \indexw{question item}} & An exercise
  consists of several questions (see Figure \ref{fig-question}). Each question contains several
  question items.\\

\midrule

\parbox[t]{3cm}{\indexw{sentence unit}\par \indexw{quiz object}\par question
...
\indexw{display feature} & An Emdros feature presented to the user as part of a question item  (see Figure
\ref{fig-question}).\\

\midrule

\indexw{request feature} & An Emdros feature which the user must provide as part of a question item
(see Figure \ref{fig-question}).\\

\midrule

\parbox[t]{3cm}{\indexw{passages}\par \indexw{universe}} & The collection of Bible passages from
which an exercise draws its sentences. The term ``universe'' is found in older parts of the code.\\

\midrule

\indexw{grammar selection box} & A box found on the left part of the display where the user can select what
in-line grammatical features to display. See Figure \ref{fig-grammar-info}.\\

\midrule

\indexw{grammar information box} & A box found on the right part of the display containing grammatical
information about the word under the mouse pointer. See Figure \ref{fig-grammar-info}.\\

\midrule

\indexw{grammar hierarchy}\index{hierarchy, grammar} & The organization of components of text. At
...
\caption{A \indexw{question} containing two \indexw{question item}s.}\label{fig-question}
\end{figure}

\begin{figure}[h]
\includegraphics[width=\textwidth]{grammarboxes.png}
\caption{The \indexw{grammar selection box} and the \indexw{grammar information box}.}\label{fig-grammar-info}
\end{figure}
...
\section{\indexw{PLOTLearner}: A EuroPLOT Product (2011-2013)}
...
\section{\indexw{Bible Online Learner}: Web-based (since 2013)}

In 2013 I started moving PLOTLearner from a Java-based stand-alone PC application to a web-based
solution. Once again, the name of the product was change once more and became \emph{Bible Online Learner},
or \emph{Bible OL} for short. This is product that is described in this document.
...
the system could also be set up on a Windows computer, but I have not tried it and you will not find
any instructions how to do it here.

The following should be installed:

\begin{itemize}
\item \indexw{Apache} (\indexw{web server}). I use version 2.4.7, but version 2.2.22 also works.
\item \indexw{MySQL} (database system). I use version 5.5.41.
...
\item (Development system only:) \indexw{nodejs} (JavaScript runtime, used for compiling
  TypeScript and Less). I use version 0.10.25.
\item (Development system only:) \indexw{npm} (package manager for nodejs). I use version 1.3.10.
  This package may not strictly be required, but it is useful for installing nodejs modules.
\item (Development system only:) \indexw{git}. I use version 1.9.1.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%% Installing Emdros %%%%%%%%%%%%%%%%%%%%
\section{Installing Emdros}

\subsection{Emdros on a Development System}\label{php-mql}
...
%%%%%%%%%%%%%%%%%%%% The visual Feature %%%%%%%%%%%%%%%%%%%%
\section{The \emph{visual} Feature}\index{visual}
...
\subsection{What Is a Word?}\label{contin}
...
%%%%%%%%%%%%%%%%%%%% Database Specification File: XXX.db.json %%%%%%%%%%%%%%%%%%%%
...
Bible OL uses the information in \emph{sentencegrammar} in two locations. One is the \indexw{grammar
selection box} in the left part of the screen, the other is in the \indexw{grammar information box} in the
...
In the above illustration, each of the four main boxes, \emph{Word, Phrase, Clause,} and
...
%%%%%%%%%%%%%%%%%%%% Database Localization Files: XXX.LANG.prop.json %%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%% Database Type Information Files: XXX.typeinfo.json %%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%% Database Book Order Files: XXX.bookorder %%%%%%%%%%%%%%%%%%%%
...
This will yield a collection of twelve possible pronominal suffixes associated with the \emph{lex}
...
that sheaf, the so-called ``\indexw{quick harvest}\index{harvest, quick}'' method can be used. In
...
% ispell-extra-args: ("--home-dir=/home/claus/Projects/3ET.web/techdoc")
