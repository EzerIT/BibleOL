...
zocial\index{zocial} & Icon\index{icon} and styles for setting up a Google
login\index{login!Google} button.
...
\ref{sec-statistics-tables}) may still contain data from these older databases. In order to display
these statistics, Bible OL needs description and localization information for the databases. For
this reason, Database Description Files for the old databases are still present in the \texttt{db}
directory.
...
google\_client\_id & Used by Google login\index{login!Google} (see Section \ref{sec-google-login}).\\

google\_client\_secret & Used by Google login\index{login!Google} (see Section \ref{sec-google-login}).\\
\end{my-tabu}
...
google & Handles login using a Google\index{login!Google} account.\\
...
%%%%%%%%%%%%%%%%%%%% Google Login %%%%%%%%%%%%%%%%%%%%
\section{Google Login}\label{sec-google-login}\index{login!Google|hyperit}

The server provides two different login mechanism. One uses a local\index{login!local} list of users
(see Section \ref{sec-user-table}), the other relies on a user's Google login. Google's description
of how login works can be found here:
\url{https://developers.google.com/accounts/docs/OAuth2WebServer}. The following is a brief
description of the mechanism as it is set up on the Bible OL installation that runs at
\url{http://bibleol.3bmoodle.dk}.
...
is set to \emph{false} (0).\index{login!local}

Google users have usernames such as ``ggl\_106440263559736360192'', where 106440263559\-736\-360192 is
the user ID provided by Google. Their \emph{google\_login} field is set to \emph{true} (1).\index{login!Google}

The \emph{may\_see\_wivu} was used previously for the WIVU database, which was available in two
versions: the entire Old Testament and a subset containing about 20\% of the text.\index{WIVU}
...
\section{Internationalization of the Client Code}
...
%%%%%%%%%%%%%%%%%%%% The SHEBANQ Website %%%%%%%%%%%%%%%%%%%%
\section{The SHEBANQ Website}\index{SHEBANQ}
...
The features \emph{english} and \emph{german} give English and German glosses for the
lexeme.\index{lexeme} The string may contain several meanings separated by commas and semicolons.
Also, special information may be given between < and > characters. For example:
